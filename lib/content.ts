...
//
// Every exported query function here is async on purpose, even though it
// currently just reads from the static arrays below. That means the call
// sites (Server Components under app/) never need to change when this file
// is rewritten to query Supabase instead — only this file does. See
// db/schema.sql for the matching Postgres schema.

import type { Article, Author, DonationSplit, Issue, Journal, MembershipTier, Topic } from './types'
...
const authorBySlug: Record<string, Author> = Object.fromEntries(authors.map((a) => [a.slug, a]))
...
const topicBySlug: Record<string, Topic> = Object.fromEntries(topics.map((t) => [t.slug, t]))
...
// --- Query functions -------------------------------------------------
// All async: today they resolve synchronously against the arrays above,
// but the signature is already what a Supabase-backed implementation
// would need, so callers never have to change.
...
export async function getAuthorsForArticle(article: Article): Promise<Author[]> {
  return article.authorSlugs.map((s) => authorBySlug[s]).filter((a): a is Author => Boolean(a))
}
