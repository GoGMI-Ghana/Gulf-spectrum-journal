    ...
    <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <aside className="space-y-8">
            <div className="border-l-4 border-gold bg-royal-blue text-white p-6">
              ...
              <Link href="/about" className="text-gold text-sm font-medium hover:underline">
                Learn more →
              </Link>
            </div>
            ...
            <div className="border-l-4 border-royal-blue p-6">
              ...
              <Link href="/submissions" className="text-ocean-blue text-sm font-medium hover:underline">
              ...
              <ul className="space-y-2.5 text-sm">
                {topics.map((topic) => (
                  <li key={topic.slug} className="pb-2.5 border-b border-slate-200 last:border-0 last:pb-0">
                    <Link href={`/topics/${topic.slug}`} className="text-slate-700 hover:text-ocean-blue">
                      {topic.label}
                    </Link>
                  </li>
                ))}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="flex items-end justify-between mb-6 border-b-2 border-royal-blue pb-3">
          ...
          <Link href="/topics" className="text-ocean-blue text-sm font-medium hover:underline">
            All topics →
          </Link>
        </div>
        <div className="flex flex-wrap gap-2.5">
          {topics.map((topic, i) => (
            ...
            >
              {topic.label} <span className="text-slate-400">({topicCounts[i].length})</span>
            </Link>
          ))}
        </div>
      </section>
